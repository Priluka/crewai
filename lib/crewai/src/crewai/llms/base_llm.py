...
class BaseLLM(ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.callbacks = [] #ADDED BY TEAMORA
    ...
    def _track_token_usage_internal(self, usage_data: dict[str, Any]) -> None:
        ...
        # TEAMORA - ADD THIS LINE at the end:
        self._trigger_token_callbacks(prompt_tokens, completion_tokens, cached_tokens)
    ...
    def _trigger_token_callbacks(
      self,
      prompt_tokens: int,
      completion_tokens: int,
      cached_tokens: int = 0,
    ) -> None:
      """Trigger token usage callbacks for external tracking (e.g., Redis)."""

      if not hasattr(self, 'callbacks') or not self.callbacks:
        return

      # Try to import litellm Usage, fallback to mock
      try:
        from litellm.types.utils import Usage

        # Create a real Usage object
        usage = Usage(
        ...
        if cached_tokens and hasattr(usage, 'prompt_tokens_details'):
          usage.prompt_tokens_details = type('Details', (), {'cached_tokens': cached_tokens})()
      except ImportError:
        ...
        class MockUsage:
          def __init__(self, prompt: int, completion: int, cached: int):
            self.prompt_tokens = prompt
            self.completion_tokens = completion
            self.total_tokens = prompt + completion
            self.prompt_tokens_details = MockDetails(cached) if cached else None

        usage = MockUsage(prompt_tokens, completion_tokens, cached_tokens)
